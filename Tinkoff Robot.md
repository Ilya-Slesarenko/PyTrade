#Робот для автоматизации работы с портфелем с помощью API Тинькоф

###Модули для работы бота - импорт
```{Python}
import os
import json
...
###Логирование - устаноление формата
```
...
###Функция определения базовых [FIGI ("Фиджи")](https://investfunds.ru/glossary/kody-figi-figi-composite/).
```{Python}
def get_strategy():
    strategy_json = """
    {
...
###Определение целевых значений по бумаге, и вектора действий внутри позиции
```{Python}
...
###Основной класс рабочего приложения
```{Python}
...
```
>Проверка входных данных на консистентность, </br>проверка на неотрицательность
```    
